...
一直想做一款大学社团管理系统，看了很多优秀的开源项目但是发现没有合适的。于是利用空闲休息时间开始自己写了一套管理系统。学习过程中遇到问题可以咨询微信：lengqin1024 或者发Email: kefu308@gmail.com

## 在线体验


- 代码下载：https://github.com/geeeeeeeek/team
...
- 社团管理：管理系统可以录入、修改和查询社团的基本信息，如社团名称、简介、备注等。
- 类型管理：系统可以管理社团的类型信息，包括类型的名称等。
- 标签管理：管理系统科研录入、修改和查询标签的信息。
...
```
CREATE DATABASE IF NOT EXISTS java_team DEFAULT CHARSET utf8 COLLATE utf8_general_ci
```
(4) 恢复sql数据。在mysql下依次执行如下命令：

```
mysql> use java_team;
mysql> source D:/xxx/xxx/java_team.sql;
```
...
在社团管理系统中，需要存储和管理社团信息、成员信息、分类信息、标签信息、用户信息、通知信息、日志信息。
...
Table thing {
    ...
    title varchar
    cover varchar  // 封面
    zongzhi varchar // 社团宗旨
    status int  
    description text
    mobile varchar 
    email varchar // 邮箱
    location varchar  // 社团地址
...
无论是社团管理、用户管理、成员管理、分类管理、评价管理、日志管理、消息管理等功能都是基于springboot+vue框架开发的，开发流程是：
      ...
      >
        <div>
          <a-form ref="myform" :label-col="{ style: { width: '80px' } }" :model="modal.form" :rules="modal.rules">
            <a-row :gutter="24">
              <a-col span="24">
                <a-form-item label="名" name="title">
